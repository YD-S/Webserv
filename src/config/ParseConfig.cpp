...
void ParseConfig::parseConfig() {
    ...
    std::vector<std::pair<std::string, std::string> > locationConf;
    config = processFile();
    ...
    std::string lastVar;
    for (std::vector<std::pair<std::string, std::string> >::iterator it = config.begin();
         it != config.end(); it = config.erase(it)) {
        ServerConfig server;
        pastString.clear();
        pastString = it->first;
        if ((it + 1) == config.end())
        ...
        if (pastString == "server" && currentString == "{") {
            for (; it->first != "}"; it = config.erase(it)) {
                ...
                if (it->first == "location") {
                    LocationConfCopy(it, config, locationConf);
                    continue;
                }
                if (it->first == "}")
                    break;
                ...
                pastString = it->first;
                it = config.erase(it);
                for (; it != config.end() && it->first != "}" && it->first != "{" &&
                       it->first != ";"; it = config.erase(it)) {
        ...
        for (std::vector<std::pair<std::string, std::string> >::iterator iter = locationConf.begin();
             iter != locationConf.end() && iter->first != "}"; iter = locationConf.erase(iter)){
            if (iter->first == "location")
                iter = locationConf.erase(iter);
            server.addLocation(parseLocation(iter, locationConf, server.getDefaultLocation()));
        }
...
void ParseConfig::validate_braces(std::ifstream &file) {
        ...
        for (size_t i = 0; i < line.size(); i++) {
            if (line[i] == '{') {
                ...
                if (brace_stack.empty()) {
                    ft_error("Error: Unmatched closing brace at line " + to_string(line_number) + ", column " +
                             to_string(i + 1), 1);
                } else {
                    brace_stack.pop();
                }
    ...
    while (!brace_stack.empty()) {
        ...
        ft_error("Error: Unmatched opening brace at line " + to_string(position.first) + ", column " +
                 to_string(position.second), 1);
    }
}

LocationConfig ParseConfig::parseLocation(std::vector<std::pair<std::string, std::string> >::iterator &it,
...
void ParseConfig::LocationConfCopy(std::vector<std::pair<std::string, std::string> >::iterator &it,
                                   std::vector<std::pair<std::string, std::string> > &config,
                                   std::vector<std::pair<std::string, std::string> > &configCopy){
    for (; it != config.end() && it->first != "}"; it = config.erase(it)) {
        configCopy.push_back(std::make_pair(it->first, it->second));
    }
    configCopy.push_back(std::make_pair(it->first, it->second));
}

void ParseConfig::prepareLocation(std::vector<std::pair<std::string, std::vector<std::string> > > &variables,
                                  LocationConfig &location) {
    std::vector<std::pair<std::string, std::vector<std::string> > >::iterator temp;
    std::string nonVectorVars[] = {
            "autoindex",
            "directory_listing",
            "directory_response_file",
            "cgi_enabled",
            "upload_enabled",
            "upload_path",
            "redirect",
    ...
    std::string vectorVars[] = {
            "index",
            "method",
            "cgi",
            "error_page"
    };
    void (LocationConfig::*functionArray[])() = {&LocationConfig::clearIndex, &LocationConfig::clearMethods,
                                                 &LocationConfig::clearCgi, &LocationConfig::clearErrorPages};
    for (std::vector<std::pair<std::string, std::vector<std::string> > >::iterator it = variables.begin();
         it != variables.end(); ++it){
        for (temp = it + 1; temp != variables.end(); ++temp) {
            if (temp->first == it->first) {
                for (int i = 0; i < 8; i++) {
                    if (temp->first == nonVectorVars[i]) {
                        temp = variables.erase(temp);
                        LOG_WARNING(nonVectorVars[i] + " was declared again. Copying only the first one.");
                        break;
                    }
                }
            }
    ...
    for (std::vector<std::pair<std::string, std::vector<std::string> > >::iterator it = variables.begin();
         it != variables.end(); ++it) {
        for (int i = 0; i < 4; i++) {
            if (it->first == vectorVars[i]) {
                (location.*functionArray[i])();
                break ;
            }
        }
    }
...
void ParseConfig::LocationSetter(std::vector<std::pair<std::string, std::vector<std::string> > > &variables,
                                 LocationConfig &location) {
    prepareLocation(variables, location);
    std::vector<std::pair<std::string, std::vector<std::string> > >::iterator temp;
    ...
    for (; it != variables.end(); it = variables.erase(it)) {
        ...
        if (it->first == "root") {
            ...
            location.clearIndex();
            for (std::vector<std::string>::iterator iter = it->second.begin();
                 iter != it->second.end(); iter = it->second.erase(iter)) {
            ...
            try {
                ...
                LOG_ERROR("Error code not valid!");
                exit_handler(1);
            }
            iter = it->second.erase(iter);
            ...
            try {
                ...
                LOG_ERROR("Max body size not valid!");
                exit_handler(1);
            }
...
void ParseConfig::mainSetter(std::vector<std::pair<std::string, std::vector<std::string> > > &variables,
    ...
    prepareLocation(variables, defaultLocation);
    std::vector<std::pair<std::string, std::vector<std::string> > >::iterator temp;
    std::vector<std::pair<std::string, std::vector<std::string> > >::iterator it = variables.begin();
    for (; it != variables.end(); it = variables.erase(it)) {
        if (it->first == "server_name") {
            ...
            for (; temp != variables.end(); ++temp)
                if (temp->first == "server_name")
                    ft_error("server_name was declared more than once. Exiting...", 1);
            ...
            if (!it->second.empty())
                LOG_WARNING("root has more than one value. Setting only the first one.");
            // make it iterate through all locations in the vector and set root for each if not set
            std::vector<LocationConfig> locations = server.getLocations();
            for (std::vector<LocationConfig>::iterator locationIt = locations.begin(); locationIt != locations.end(); ++locationIt) {
                if ((*locationIt).getRoot().empty()) {
                    (*locationIt).setRoot(defaultLocation.getRoot());
                }
            }
        } else if (it->first == "index") {
            for (std::vector<std::string>::iterator iter = it->second.begin();
            ...
            if (!it->second.empty())
                LOG_WARNING("autoIndex has more than one value. Setting only the first one.");
            // make it iterate through all locations in the vector and set autoindex to true if this one is true
    ...
    if (defaultLocation.getIndexes().empty()){
        defaultLocation.addIndex("index.html");
        LOG_WARNING("No index found in server " + server.getServerName() +". Setting index.html as default index file");
    }
...
void ParseConfig::printLocation(const LocationConfig &location) {
    ...
    std::cout << "  Cgi: ";
    for (std::vector<std::pair<std::string, std::string> >::const_iterator cgiIt = location.getCgi().begin();
         cgiIt != location.getCgi().end(); ++cgiIt) {
    ...
    std::cout << "  ErrorPages: ";
    for (std::vector<std::pair<int, std::string> >::const_iterator errorPageIt = location.getErrorPages().begin();
         errorPageIt != location.getErrorPages().end(); ++errorPageIt) {
...
void ParseConfig::printAll() {
    for (std::vector<ServerConfig>::iterator serverIt = _servers.begin(); serverIt != _servers.end(); ++serverIt) {
        ...
        std::cout << "  Listen: ";
        for (std::vector<std::pair<std::string, int> >::const_iterator listenIt = server.getListen().begin();
             listenIt != server.getListen().end(); ++listenIt) {
