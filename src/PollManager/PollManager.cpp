...
void PollManager::poller() {
    ...
    readRequests(read_fd);
    std::vector<const HttpResponse *> responsesSent = sendResponses(write_fd);
    removeSentResponses(responsesSent);
}

void PollManager::readRequests(fd_set &read_fd) {
    for (unsigned long i = 0; i < sockets.size(); i++) {
...
std::vector<const HttpResponse *> PollManager::sendResponses(fd_set &write_fd) {
...
void PollManager::removeSentResponses(const std::vector<const HttpResponse *> &responsesSent) {// This can probably be optimized
